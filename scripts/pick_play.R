...
plot_play <-
  function(game_id = 2018090600,
    ...
    route_frames <-
      ...
      dplyr::filter(group == 1L) %>%
      dplyr::select(-group) %>% 
      select(game_id, play_id, nfl_id, frame_id, event, side, jersey_number, x, y, o) %>% 
      arrange(game_id, play_id, nfl_id, frame_id)
    
    snap_frames <- route_frames %>% filter(frame_id == min(frame_id))
    end_frames <- route_frames %>% filter(frame_id == max(frame_id)) %>% mutate(event = !!event_end)
    ...
    ball <-
      ball %>% 
      select(-event) %>% 
      inner_join(frames %>% distinct(game_id, play_id, frame_id, event), by = c("frame_id", "game_id", "play_id")) %>% 
      select(game_id, play_id, nfl_id, frame_id, event, x, y)
    ...
    p <-
      ...
      ggplot2::facet_wrap(
        ...
        nrow = 2L, 
        strip.position = "left", 
        labeller = ggplot2::labeller(event = function(x) ifelse(x == "ball_snap", "Snap", "End of Routes"))
      ) +
      ggplot2::aes(x = x, y = y, group = nfl_id) +
      ggplot2::geom_segment(
        data = tibble::tibble(x = line_of_scrimmage, event = events),
